...
- `scripts/`
  ...
  - `$start-repository` skill は `scripts/start_repository.sh` を呼び、その wrapper が clean clone では init 前の `make agent-canon-ensure-latest`、`scripts/init_from_template.sh`、`tools/update_agent_canon.sh register-local-bare`、必要な post-commit validation をまとめます。`--force` を init に渡すと wrapper preflight は block 扱いで skip し、dirty override を邪魔しません。
- `docker/`
...
- `make agent-canon-register-local-bare ARGS='--bare-repo /mnt/git/<slug>-agent-canon.git'`
  - project-local `agent-canon` bare repo を seed して `agent-canon` remote を登録します。daily validation の正本はこの project-local remote に戻し、shared upstream refresh を残したいときだけ `ARGS='--bare-repo ... --source-repo /path/to/agent-canon'` を明示します。
- `make agent-checks`
...
AgentCanon の URL や branch 情報が `.gitmodules` と submodule config でずれた場合は `git submodule sync vendor/agent-canon` を先に実行します。submodule worktree が stale / detached / local-only commit を含む場合は、親 repo の tree diff ではなく `bash tools/update_agent_canon.sh review-submodule` で proposal 要否、conflict、safe align 可否を確認します。前回の proposal が GitHub `main` に取り込まれていると判定された場合だけ `bash tools/update_agent_canon.sh align-main` で main に揃えます。
...
- `QUICK_START.md`
- `documents/project-template-overview-slides.md`
...
初期化時には project-local `agent-canon` bare repo と、その repo 専用の proposal branch `canon-proposal/<project-slug>` も用意されます。register-local-bare は既定で project-local remote を daily validation の正本にし、shared canon の差分は proposal branch に push します。shared upstream refresh を使いたい場合だけ `agent-canon.sourceRepo` を明示設定します。
...
VS Code の dev container は `.devcontainer/` から起動します。起動時の compose 生成は `python3 tools/ci/render_devcontainer_compose.py --pack docker/packs/default.toml` を正本にし、GPU が見える host では `gpus: all` を自動追加し、GPU が無い host では CPU-only で起動します。`/mnt/git` も host に path がある場合だけ mount し、local bare remote への push/pull を container 内から継続できます。host `~/.codex`、`~/.config/gh`、`~/.ssh` があれば container へ mount し、`SSH_AUTH_SOCK` が有効なら `/ssh-agent` として forward します。初回 `codex login`、`gh auth login`、SSH host key 登録は host 側で行い、container はその state を再利用します。attach 直後には banner を出し、GPU、`/mnt/git`、host auth mount、Codex login、SSH agent、Docker socket、Codex の `approval_policy` / `sandbox_mode` の状態を表示します。前提拡張は `.vscode/extensions.json` にまとめています。
