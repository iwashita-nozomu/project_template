...
- `experiments/`
  ...
  - topic 一覧は `experiments/registry.toml`、topic template source は `vendor/agent-canon/templates/experiments/_template/`、run report は `experiments/report/` に置きます。
- `python/`
...
server で回す実験コードの実体テンプレ source は `vendor/agent-canon/templates/experiments/_template/`、topic 正本は `experiments/registry.toml`、topic scaffold は `tools/agent-canon/experiments/create_experiment_topic.py`、run metadata を残す入口は `tools/agent-canon/experiments/run_managed_experiment.py` です。
...
repo-wide な tool 導入案や Docker 変更では `vendor/agent-canon/templates/agents/environment_change_proposal.md` に triggering code requirement、blocked command、Docker 影響、validation、rollback を残します。
