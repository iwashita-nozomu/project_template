...
| `manager_reviewer` | parent review gate or `reviewer` |
...
- `document_flow_reviewer` は `detailed_design_reviewer` と同じ instance を再利用しません
- `notation_definition_reviewer` は `document_flow_reviewer` や `logic_gap_reviewer` と同じ instance を再利用しません
- `logic_gap_reviewer` は `document_flow_reviewer` や `notation_definition_reviewer` と同じ instance を再利用しません
- `requirements_organizer`、`execution_planner`、`detailed_designer`、`worker` の担当を 1 つの instance にまとめません
- 詳細設計レビューが unresolved のまま `worker` を起動しません
- 文書通読レビューで major rewrite 指摘が残っている間は `worker` を起動しません
- 包括的開発では、同一 worktree の write role を `worker` 1 人に固定します
- 包括的開発では、同一 worktree の parallel write を禁止します
- 複数 writer が必要な場合は worktree を分け、各 worktree に `worker` を 1 人だけ置きます
- parent は worktree ごとの結果を順番に統合し、未統合 patch を抱えたまま次段へ進みません
