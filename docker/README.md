...
- `Dockerfile`
  - canonical container image 定義です。OS package、Node.js、Codex CLI、GitHub CLI までを入れ、workspace-mounted Python module は build 時に入れません。
- `requirements.txt`
...
`vendor/agent-canon` submodule や workspace Python package に依存しない OS / CLI layer
...
GitHub CLI は `docker/Dockerfile` で canonical image に同梱します。初回認証は host で人間が行います。
