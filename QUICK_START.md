...
bash tools/agent-canon/run_comprehensive_review.sh
```
...
- 新しい topic は `python3 tools/agent-canon/experiments/create_experiment_topic.py <topic>` で作ります。
- 実行ごとの生成物は `experiments/<topic>/result/<run_name>/` に置きます。
- 1 回の実験 report は `experiments/report/<run_name>.md` に置きます。
- server で formal run を回すときは `tools/agent-canon/experiments/run_managed_experiment.py --topic <topic> --use-registered-command formal` で `run_manifest.json` と `run.log` を残します。
...
- Python 依存を追加する場合は `pyproject.toml` の project dependency または `dev` extra だけを更新し、`pip-compile --all-build-deps --allow-unsafe --extra=dev --generate-hashes --output-file=docker/requirements.txt pyproject.toml` で lock を再生成します。導入は post-create / runtime setup の `docker/install_python_dependencies.sh` に集約します。
...
bash tools/agent-canon/push_origin.sh
```

- 長期に残す知見は `notes/` に寄せます。
- worktree を使った場合は、`vendor/agent-canon/documents/operations/notes-lifecycle.md` を見て action log から knowledge/theme/failure へ昇格させる項目を決めます。
